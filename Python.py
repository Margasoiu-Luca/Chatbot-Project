...
Sentence=input("Hello! My name is Mr Chatbot! What is your name?") #
    ...
    if Words[i][0].isupper(): #If the first letter is capital then it means it's a name
        try: #We try to fetch the data
            ...
            if(Data['likelyGender']=='male'): # Check for male or female, information which was sent in the response
                x = input("Nice to meet you "+Words[i]+"!I think you are a boy! Is that correct?")
            else:
                x = input("Nice to meet you "+Words[i]+"!I think you are a girl! Is that correct?")
            if x[0].lower()=='y':
                print("I knew I was correct!")
            else:
                print("I'm so sorry! I will know next time, trust me!")
